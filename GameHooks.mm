...
uintptr_t FindExecutableBase() {
  for (uint32_t i = 0; i < _dyld_image_count(); ++i) {
    ...
    if (header != nullptr && header->filetype == MH_EXECUTE) {
      // The recovered executable uses a zero VM base. dyld's header pointer is
      // already relocated, so it is the runtime base used for these RVAs.
      ...
      for (uint32_t commandIndex = 0; commandIndex < header64->ncmds;
        ...
        if (command->cmd == LC_SEGMENT_64) {
          ...
          if (std::strncmp(segment->segname, "__TEXT", sizeof(segment->segname)) == 0) {
            // Recovered RVAs are relative to the loaded Mach header, not the
            // unslid LC_SEGMENT_64 vmaddr (commonly 0x100000000 for apps).
